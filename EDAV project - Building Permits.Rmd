...
Then we filtered Manhattan Zip codes that are within the area of influence of phase1 and phase 2 of the project (10065, 10021, 10075, 10028, 10128, 10029, 10036) and selected relevant columns of our interest:
...
For this purpose, we first summarize by Filing.Date year and Distance.

After doing so, we scale the data and create an index to visualize variations over time with time series lines for recent years:
...
Initially, we plotted just recent years but we noted that the category '3-4 blocks right from 2nd Ave line' had higher increase in variation than other lines which was not an expected result.

However, after plotting from the initial year when data is available (1990) we can see that historically, that block of buildings have always had a higher increase in the number of building permits, which we think is due to the fact that this last category is right on the edge of the East River, which one may expect that would attract a higher number of projects than the other categories, due to the water views. 
